...
import { ref, set, serverTimestamp, get, query, orderByChild, equalTo } from 'firebase/database';
...
export default function Login() {
  ...
  const assertNicknameAvailable = async (uid: string, nick: string) => {
    try {
      const snap = await get(query(ref(db, '/profiles'), orderByChild('nickname'), equalTo(nick)));
      if (!snap.exists()) return;
      const entries = snap.val() as Record<string, { uid?: string }>;
      const conflict = Object.values(entries).some((item) => item?.uid && item.uid !== uid);
      if (conflict) {
      ...
      if (err?.code === 'nickname-taken') throw err;
      const error = new Error('nickname-check-failed');
  ...
  const afterLogin = async (
    ...
    try {
      await assertNicknameAvailable(user.uid, normalizedNickname);
