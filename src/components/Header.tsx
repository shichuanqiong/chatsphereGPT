  ...
  return (
    <>
      ...
      <header
        id="topbar"
        ref={topbarRef}
        className="app-header sticky top-0 z-40 bg-black/40 backdrop-blur-xl border-b border-white/10 px-3 md:px-6"
          ...
          <div id="rightControls" className="header-right justify-self-end flex items-center gap-2 min-w-0">
            ...
            >
              ...
              {menuOpen && (
                <div
                  id="profileMenu"
                  className="avatar-menu absolute right-0 top-[calc(100%+8px)] z-[60] w-[66vw] max-w-[240px] md:w-[300px] md:max-w-none rounded-2xl bg-black/80 backdrop-blur-md shadow-xl border border-white/10 p-3 select-none"
                  onMouseEnter={openMenuHover}
                  onMouseLeave={scheduleCloseMenu}
                >
                  {/* 第一行：Edit / Profile 居中紧凑 */}
                  <div className="grid grid-cols-2 gap-1 mb-2 place-items-center text-center">
                    <button
                      className="px-3 py-2 rounded-xl text-white/90 hover:bg-white/10 active:bg-white/15 mx-auto transition-colors"
                      onClick={() => {
                        setMenuOpen(false);
                        setOpenProfile(true);
                      }}
                  ...
                  {/* 第二行：Logout 居中 */}
                  <button
                    className="px-4 py-2 rounded-xl text-red-300 hover:bg-red-300/10 active:bg-red-300/15 block mx-auto transition-colors"
                    onClick={() => {
                      setMenuOpen(false);
                      onLogout();
                    }}
