    ...
    const off = onValue(inboxRef, snap => {
      ...
      if (snap.exists()) {
        snap.forEach(child => {
          ...
          if (item.unread === true && item.archived !== true && (item.type === 'dm' || item.type === 'room_invite')) {
            if (item.type === 'dm') {
              count += (item.count || 1);
            } else {
              count += 1; // room_invite 计 1 条
            }
          }
        });
      }

      // 只在「未读数增加」时 ding（边沿检测）
      const prev = prevRef.current;
      const increased = count > prev;

      // 节流（避免连环 ding）
      const now = Date.now();
      if (increased && now - lastDingRef.current > 1000 && !document.hidden) {
        Sound.play('ding'); // 用新Sound
